...
export const normalizeCapacityScore = (
  ...
  if (__DEV__) {
    const rawPercent = target > 0 ? (rawScore / target) * 100 : 0;
    console.log("[CAPACITY_DEBUG] normalizeCapacityScore:", {
      capacityKey,
      rawScore,
      rawScoreType: typeof rawScore,
      athleteLevel,
      athleteLevelType: typeof athleteLevel,
      mode,
      target,
      isKnown,
      rawPercent: rawPercent.toFixed(2),
      clampedPercent: target > 0 ? clamp(rawPercent) : 0,
    });
  }
