 ...
 *
 * return <CapacityWidget variant="bars" mode={mode} items={items} />;
 * ```
 */
export const useCapacities = (options?: UseCapacitiesOptions): UseCapacitiesReturn => {
  ...
  const fetchCapacities = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) {
        console.log("[CAPACITY_DEBUG] useCapacities - profile.career:", {
          career: profile.career,
          levelRaw: profile.career?.level,
          levelType: typeof profile.career?.level,
          levelExtracted: level,
          levelExtractedType: typeof level,
        });
        console.log("[CAPACITY_DEBUG] useCapacities - profile.capacities:", {
          count: profile.capacities?.length ?? 0,
          sample: profile.capacities?.slice(0, 2),
        });
      }
